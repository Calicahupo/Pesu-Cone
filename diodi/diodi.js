...
OM.re.Names.Fxsw1L=OM.re.Names.Fxsw1.length; OM.re.Names.fxsw1L=OM.re.Names.fxsw1.length; OM.re.Names.xxsx2L=OM.re.Names.xxsx2.length; OM.re.Names.FKSw1L=OM.re.Names.FKSw1.length; OM.re.Names.FkSw1L=OM.re.Names.FkSw1.length; OM.re.Names.FxxW1L=OM.re.Names.FxxW1.length; OM.re.Names.fKSw1L=OM.re.Names.fKSw1.length; OM.re.Names.fkSw1L=OM.re.Names.fkSw1.length; OM.re.Names.fxxW1L=OM.re.Names.fxxW1.length; OM.re.Names.xxSx2L=OM.re.Names.xxSx2.length;
...
function mkName2(o,fem,kid,fin,weird,last,s){ o.fem =fem; o.kid=kid; o.fin=fin; o.weird=weird;
	if 		(!fem && 		 fin &&  weird) { o.first =OM.re.Names.fxxW1[Math.floor(Math.random()*OM.re.Names.fxxW1L)]}
	else if (!fem && !kid && fin && !weird) { o.first =OM.re.Names.fkSw1[Math.floor(Math.random()*OM.re.Names.fkSw1L)]}
	else if (!fem &&  kid && fin && !weird) { o.first =OM.re.Names.fKSw1[Math.floor(Math.random()*OM.re.Names.fKSw1L)]}
 	else if (!fem &&  		!fin &&  weird) { o.first =OM.re.Names.fxxW1[Math.floor(Math.random()*OM.re.Names.fxxW1L)]}
 	else if (!fem &&   		!fin && !weird) { o.first =OM.re.Names.fxsw1[Math.floor(Math.random()*OM.re.Names.fxsw1L)]}
 	else if ( fem && 		 fin &&  weird) { o.first =OM.re.Names.FxxW1[Math.floor(Math.random()*OM.re.Names.FxxW1L)]}
	else if ( fem && !kid && fin && !weird) { o.first =OM.re.Names.FkSw1[Math.floor(Math.random()*OM.re.Names.FkSw1L)]}
	else if ( fem &&  kid && fin && !weird) { o.first =OM.re.Names.FKSw1[Math.floor(Math.random()*OM.re.Names.FKSw1L)]}
 	else if ( fem &&  		!fin &&  weird) { o.first =OM.re.Names.FxxW1[Math.floor(Math.random()*OM.re.Names.FxxW1L)]}
 	else if ( fem &&   		!fin && !weird) { o.first =OM.re.Names.Fxsw1[Math.floor(Math.random()*OM.re.Names.Fxsw1L)]}
 	if(last){ o.last =last}else{if(fin){ 	o.last  =OM.re.Names.xxSx2[Math.floor(Math.random()*OM.re.Names.xxSx2L)]}
 								else{ 		o.last 	=OM.re.Names.xxsx2}}
...
function precalcDefaultSubjs(){	var r =[]; var b =null;	
	b =mkRelSubj( "subj_A",AL.r, 1,1,1);r.push( b);
	b =mkRelSubj( "subj_a",AL.r, 1,1,0);r.push( b);
	b =mkRelSubj( "subj_B",AL.r, 0,1,1);r.push( b);
	b =mkRelSubj( "subj_b",AL.r, 0,1,0);r.push( b);
	b =mkRelSubj( "subj_C",AL.r, 1,0,1);r.push( b);
	b =mkRelSubj( "subj_c",AL.r, 1,0,0);r.push( b);
	b =mkRelSubj( "subj_D",AL.r, 0,0,1);r.push( b);
	b =mkRelSubj( "subj_d",AL.r, 0,0,0);r.push( b);
	r =precalcFamilies( r);
	b =mkRelSubj( "subj_E",AL.r, 1,1,1);r.push( b);
	b =mkRelSubj( "subj_e",AL.r, 1,1,0);r.push( b);
	b =mkRelSubj( "subj_F",AL.r, 0,1,1);r.push( b);
	b =mkRelSubj( "subj_f",AL.r, 0,1,0);r.push( b);
	b =mkRelSubj( "subj_G",AL.r, 1,0,1);r.push( b);
	b =mkRelSubj( "subj_g",AL.r, 1,0,0);r.push( b);
	b =mkRelSubj( "subj_H",AL.r, 0,0,1);r.push( b);
	b =mkRelSubj( "subj_h",AL.r, 0,0,0);r.push( b);
	return r}
function mkRelSubj( 	ni,pcReso,pcFem,pcKid,pcFin,pcTys,pcPms,pcPureBad,pcInvMor,pcPureGood,pcObj){ var b;
	if( !ni){ var ni=""}
	if( !pcReso){ 		var pcReso 	=OM.re.pcReso}
	if( !pcFem){ 		var pcFem 	=OM.re.pcFem} 	b =Math.random(); b <pcFem ? pcFem =true  : pcFem =false;
	if( !pcKid){ 		var pcKid 	=OM.re.pcKid} 	b =Math.random(); b <pcKid ? pcKid =true  : pcKid =false;
	if( !pcFin){ 		var pcFin 	=OM.re.pcFin} 	b =Math.random(); b <pcFin ? pcFin =true  : pcFin =false; 
	...
	if( !pcObj){ 		var pcObj	=OM.re.pcObj} 	b =Math.random(); b <pcObj ? pcObj =true  : pcObj =false;
	var o =new Subj( ni,pcReso,pcFem,pcKid,pcFin,pcTys,pcPms,pcPureBad,pcInvMor,pcPureGood,pcObj);
...
function mkPlace(ni,par){ var b =null; switch(ni){
	case "Sky" 			 	:b=new OM.re.Places.Sky(ni,12742000,12742000,0,10000,false,false,true,false,["Sea","Land"]); 				inhPl( b,par);return b;
	case "Sea" 		 	:b=new OM.re.Places.Sea(ni,12742000,12742000,-5000,0,false,false,true,false,["Sky","Land"]);				inhPl( b,par);return b;
	case "Land" 		 	:b=new OM.re.Places.Land(ni,7356597,7356597,-70000,5000,false,false,false,false,"Sea",["Sky","Sea"]);	inhPl( b,par);return b;
														//5385402
	case "Sandbox" 			:b=new OM.re.Places.Shelter(ni,ni,66,66,1,5,false,false,false,"Sandbox");									inhPl( b,par);return b; 			
	...
	case "TempleDoor" 	 	:b=new OM.re.Places.Gate(ni,ni,0.5,1,0,3,false,false,false,"Temple",["Garden","TempleInterior"]); 			inhPl( b,par);return b;
	...
	case "FinVilla" 	 	:b=new OM.re.Places.Area(ni,ni,100,120,3,33,true,true,true,"Land"); 										inhPl( b,par);return b;
	case "FinRoad" 	 		:b=new OM.re.Places.Way(ni,ni,50,5,3,33,false,false,true,"Land",["Intersection","FinVilla"]); 				inhPl( b,par);return b;
	case "FinHouse" 	 	:b=new OM.re.Places.Shelter(ni,ni,30,20,15,33,true,true,true,"FinVilla");									inhPl( b,par);return b;
	case "FinHall" 	 		:b=new OM.re.Places.Room(ni,ni,3,5,3,6,true,true,false,"FinHouse"); 										inhPl( b,par);return b;
	case "FinDownstairs"	:b=new OM.re.Places.Way(ni,ni,6,1.5,0,6,true,true,false,"FinHouse",["FinCellar"]);						inhPl( b,par);return b;
	case "FinCellar" 	 	:b=new OM.re.Places.Room(ni,ni,10,10,0,2.5,true,true,false,"FinHouse"); 									inhPl( b,par);return b;
	case "FinBedroom" 	 	:b=new OM.re.Places.Room(ni,ni,6,5,3,6,true,true,false,"FinHouse"); 										inhPl( b,par);return b;
	case "FinKidsRoom"  	:b=new OM.re.Places.Room(ni,ni,6,5,3,6,true,true,false,"FinHouse"); 										inhPl( b,par);return b;
	case "FinLivingRoom"	:b=new OM.re.Places.Room(ni,ni,6,5,3,6,true,true,false,"FinHouse"); 										inhPl( b,par);return b;
	case "FinFrontDoor" 	:b=new OM.re.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"FinHouse",["FinVilla","FinHall"],true);	inhPl( b,par);return b;
	case "FinCellarDoor"	:b=new OM.re.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"FinHouse",["FinHall","FinDownstairs"]); 	inhPl( b,par);return b;
	case "FinBedroomDoor"	:b=new OM.re.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"FinHouse",["FinHall","FinBedroom"]);		inhPl( b,par);return b;
	case "FinKidsRoomDoor"	:b=new OM.re.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"FinHouse",["FinHall","FinKidsRoom"]);		inhPl( b,par);return b;
	case "FinLivingRoomWay" :b=new OM.re.Places.Way(ni,ni,0.4,0.8,3,5.5,false,true,false,"FinHouse",["FinHall","FinLivingRoom"]); 	inhPl( b,par);return b;

	case "SweVilla" 	 	:b=new OM.re.Places.Area(ni,ni,100,120,3,33,true,true,true,"Land"); 										inhPl( b,par);return b;
	case "SweRoad" 	 	 	:b=new OM.re.Places.Way(ni,ni,50,5,3,33,false,false,true,"Land",["Intersection","SweVilla"]); 				inhPl( b,par);return b;
	case "SweHouse" 		:b=new OM.re.Places.Shelter(ni,ni,30,20,15,33,true,true,true,"SweVilla");									inhPl( b,par);return b;
	case "SweHall" 	 		:b=new OM.re.Places.Room(ni,ni,3,5,3,6,true,true,false,"SweHouse"); 										inhPl( b,par);return b;
	case "SweDownstairs"	:b=new OM.re.Places.Way(ni,ni,6,1.5,0,6,true,true,false,"SweHouse",["SweCellar"]);							inhPl( b,par);return b;
	case "SweCellar" 		:b=new OM.re.Places.Room(ni,ni,10,10,0,2.5,true,true,false,"SweHouse"); 									inhPl( b,par);return b;
	case "SweBedroom" 		:b=new OM.re.Places.Room(ni,ni,6,5,3,6,true,true,false,"SweHouse"); 										inhPl( b,par);return b;
	case "SweKidsRoom"  	:b=new OM.re.Places.Room(ni,ni,6,5,3,6,true,true,false,"SweHouse"); 										inhPl( b,par);return b;
	case "SweLivingRoom"	:b=new OM.re.Places.Room(ni,ni,6,5,3,6,true,true,false,"SweHouse"); 										inhPl( b,par);return b;
	case "SweFrontDoor" 	:b=new OM.re.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"SweHouse",["SweVilla","SweHall"],true);		inhPl( b,par);return b;
	case "SweCellarDoor"	:b=new OM.re.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"SweHouse",["SweHall","SweDownstairs"]);		inhPl( b,par);return b;
	case "SweBedroomDoor"	:b=new OM.re.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"SweHouse",["SweHall","SweBedroom"]);			inhPl( b,par);return b;
	case "SweKidsRoomDoor"	:b=new OM.re.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"SweHouse",["SweHall","SweKidsRoom"]); 		inhPl( b,par);return b;
	case "SweLivingRoomWay"	:b=new OM.re.Places.Way(ni,ni,0.4,0.8,3,5.5,false,true,false,"SweHouse",["SweHall","SweLivingRoom"]); 		inhPl(b,par);return b}}
function inhPl(){}
OM.re.Places = { seed: ["Sky","Sea","Land", 	"Sandbox",
