...
OM.Names.Fxsw1L=OM.Names.Fxsw1.length; OM.Names.fxsw1L=OM.Names.fxsw1.length; OM.Names.xxsx2L=OM.Names.xxsx2.length; OM.Names.FKSw1L=OM.Names.FKSw1.length; OM.Names.FkSw1L=OM.Names.FkSw1.length; OM.Names.FxxW1L=OM.Names.FxxW1.length; OM.Names.fKSw1L=OM.Names.fKSw1.length; OM.Names.fkSw1L=OM.Names.fkSw1.length; OM.Names.fxxW1L=OM.Names.fxxW1.length; OM.Names.xxSx2L=OM.Names.xxSx2.length;
...
function mkName2(o,fem,kid,fin,weird,last,s){ o.fem =fem; o.kid=kid; o.fin=fin; o.weird=weird;
	if 		(!fem && 		 fin &&  weird) { o.first =OM.Names.fxxW1[Math.floor(Math.random()*OM.Names.fxxW1L)]}
	else if (!fem && !kid && fin && !weird) { o.first =OM.Names.fkSw1[Math.floor(Math.random()*OM.Names.fkSw1L)]}
	else if (!fem &&  kid && fin && !weird) { o.first =OM.Names.fKSw1[Math.floor(Math.random()*OM.Names.fKSw1L)]}
 	else if (!fem &&  		!fin &&  weird) { o.first =OM.Names.fxxW1[Math.floor(Math.random()*OM.Names.fxxW1L)]}
 	else if (!fem &&   		!fin && !weird) { o.first =OM.Names.fxsw1[Math.floor(Math.random()*OM.Names.fxsw1L)]}
 	else if ( fem && 		 fin &&  weird) { o.first =OM.Names.FxxW1[Math.floor(Math.random()*OM.Names.FxxW1L)]}
	else if ( fem && !kid && fin && !weird) { o.first =OM.Names.FkSw1[Math.floor(Math.random()*OM.Names.FkSw1L)]}
	else if ( fem &&  kid && fin && !weird) { o.first =OM.Names.FKSw1[Math.floor(Math.random()*OM.Names.FKSw1L)]}
 	else if ( fem &&  		!fin &&  weird) { o.first =OM.Names.FxxW1[Math.floor(Math.random()*OM.Names.FxxW1L)]}
 	else if ( fem &&   		!fin && !weird) { o.first =OM.Names.Fxsw1[Math.floor(Math.random()*OM.Names.Fxsw1L)]}
 	if(last){ o.last =last}else{if(fin){ 	o.last  =OM.Names.xxSx2[Math.floor(Math.random()*OM.Names.xxSx2L)]}
 								else{ 		o.last 	=OM.Names.xxsx2}}
	o.full 	=o.first+" "+o.last; if( s){ s.nMs.push( o)} return o}

	
function precalcFamilies(a){ return a }
function precalcDefaultSubjs(sim){	var r =[]; var b =null;	
	b =mkReSubj( "subj_A",sim,YH.sQr, 1,1,1);r.push( b);
	b =mkReSubj( "subj_a",sim,YH.sQr, 1,1,0);r.push( b);
	b =mkReSubj( "subj_B",sim,YH.sQr, 0,1,1);r.push( b);
	b =mkReSubj( "subj_b",sim,YH.sQr, 0,1,0);r.push( b);
	b =mkReSubj( "subj_C",sim,YH.sQr, 1,0,1);r.push( b);
	b =mkReSubj( "subj_c",sim,YH.sQr, 1,0,0);r.push( b);
	b =mkReSubj( "subj_D",sim,YH.sQr, 0,0,1);r.push( b);
	b =mkReSubj( "subj_d",sim,YH.sQr, 0,0,0);r.push( b);
	r =precalcFamilies( r);
	b =mkReSubj( "subj_E",sim,YH.sQr, 1,1,1);r.push( b);
	b =mkReSubj( "subj_e",sim,YH.sQr, 1,1,0);r.push( b);
	b =mkReSubj( "subj_F",sim,YH.sQr, 0,1,1);r.push( b);
	b =mkReSubj( "subj_f",sim,YH.sQr, 0,1,0);r.push( b);
	b =mkReSubj( "subj_G",sim,YH.sQr, 1,0,1);r.push( b);
	b =mkReSubj( "subj_g",sim,YH.sQr, 1,0,0);r.push( b);
	b =mkReSubj( "subj_H",sim,YH.sQr, 0,0,1);r.push( b);
	b =mkReSubj( "subj_h",sim,YH.sQr, 0,0,0);r.push( b);
	return r}
function mkReSubj( 	ni,sim,pcReso,pcFem,pcKid,pcFin,pcTys,pcPms,pcPureBad,pcInvMor,pcPureGood,pcObj){ var b;
	if( !ni){ var ni=""}
	if( !pcReso){ 		var pcReso 	=OM.pcReso}
	if( !pcFem){ 		var pcFem 	=OM.pcFem} 	b =Math.random(); b <pcFem ? pcFem =true  : pcFem =false;
	if( !pcKid){ 		var pcKid 	=OM.pcKid} 	b =Math.random(); b <pcKid ? pcKid =true  : pcKid =false;
	if( !pcFin){ 		var pcFin 	=OM.pcFin} 	b =Math.random(); b <pcFin ? pcFin =true  : pcFin =false; 
	if( !pcFin){ b =Math.random();
		if( !pcPureBad){	var pcPureBad 	=OM.pcPureBad}
		if( !pcInvMor){		var pcInvMor 	=OM.pcPureBad +OM.pcInvMor}
		if( !pcPureGood){ 	var pcPureGood 	=OM.pcPureBad +OM.pcInvMor +OM.pcPureGood}
		...
		if( !pcPureGood){ 	var pcPureGood 	=OM.pcPureGood}
		if 		( b <pcPureGood){ pcPureBad=false;pcInvMor=false;pcPureGood=true }
		else 					{ pcPureBad=false;pcInvMor=false;pcPureGood=false}}

	if( !pcTys){ 			var pcTys 		=OM.pcTys}
	...
	if( !pcObj){ 		var pcObj	=OM.pcObj} 	b =Math.random(); b <pcObj ? pcObj =true  : pcObj =false;
	console.log(sim);
...
function precalcDefaultPlaces(seed,par){ var ps=[]; var p =null;
	for(var n=0; n<OM.PlacesL; n++){ p=mkPlace(seed[n],par); ps.push(p)} return ps}
function mkPlace(ni,par){ var b =null; switch(ni){
	case "Sky" 			 	:b=new OM.Places.Sky(ni,12742000,12742000,0,10000,false,false,true,false,["Sea","Land"]); 				inhPl( b,par);return b;
	case "Sea" 		 	:b=new OM.Places.Sea(ni,12742000,12742000,-5000,0,false,false,true,false,["Sky","Land"]);				inhPl( b,par);return b;
	case "Land" 		 	:b=new OM.Places.Land(ni,7356597,7356597,-70000,5000,false,false,false,false,"Sea",["Sky","Sea"]);	inhPl( b,par);return b;
														//5385402
	case "Sandbox" 			:b=new OM.Places.Shelter(ni,ni,66,66,1,5,false,false,false,"Sandbox");									inhPl( b,par);return b; 			

	case "Beach" 		 	:b=new OM.Places.Area(ni,ni,300,75,-2,4,false,false,true,"Land");
	case "Market" 		 	:b=new OM.Places.Area(ni,ni,200,50,1,5,false,false,true,"Land",["Beach"]);								inhPl( b,par);return b;
	case "Inn" 				:b=new OM.Places.Shelter(ni,ni,25,25,1,5,false,false,false,"Land"); 										inhPl( b,par);return b;
	case "InnYard" 	 		:b=new OM.Places.Area(ni,ni,25,10,1,5,false,false,true,"Land",["Beach","Inn"]);							inhPl( b,par);return b;
	case "Esplanade" 		:b=new OM.Places.Way(ni,ni,25,5,1,5,false,false,true,"Land",["Market","InnYard","Beach"]); 				inhPl( b,par);return b;
	case "InnInterior" 		:b=new OM.Places.Room(ni,ni,24,9,0,4,true,false,false,"Inn"); 											inhPl( b,par);return b;
	case "InnDoor" 	 		:b=new OM.Places.Gate(ni,ni,0.4,0.8,0,2.5,false,false,false,"Inn",["InnYard","InnInterior"]);			inhPl( b,par);return b;

	case "ObeliskYard" 		:b=new OM.Places.Area(ni,ni,40,40,8,16,false,false,true,"Land"); 										inhPl( b,par);return b;
	case "Obelisk" 			:b=new OM.Places.Area(ni,ni,1,1,15140,15240,false,false,true,"ObeliskYard"); 							inhPl( b,par);return b;
	case "BoulevardBeach" 	:b=new OM.Places.Way(ni,ni,150,10,1,16,false,false,true,"Land",["Market","ObeliskYard"]); 				inhPl( b,par);return b;
	case "Garden" 		 	:b=new OM.Places.Area(ni,ni,200,200,16,25,false,false,true,"Land"); 										inhPl( b,par);return b;
	case "Temple" 	 	 	:b=new OM.Places.Shelter(ni,ni,88,66,21,25,false,false,true,"Garden"); 									inhPl( b,par);return b;
	case "TempleInterior"	:b=new OM.Places.Room(ni,ni,87,65,0,5,false,false,false,"Temple"); 										inhPl( b,par);return b;
	case "TempleDoor" 	 	:b=new OM.Places.Gate(ni,ni,0.5,1,0,3,false,false,false,"Temple",["Garden","TempleInterior"]); 			inhPl( b,par);return b;
	case "Intersection"  	:b=new OM.Places.Area(ni,ni,10,5,16,25,false,false,true,"Land",["Garden"]);								inhPl( b,par);return b;
	case "BoulevardInland" 	:b=new OM.Places.Way(ni,ni,90,10,16,25,false,false,true,"Land",["ObeliskYard","Intersection"]); 			inhPl( b,par);return b;

	case "FlatYard" 	 	:b=new OM.Places.Area(ni,ni,60,50,2,25,true,true,true,"Land",["ObeliskYard"]); 							inhPl( b,par);return b;
	case "Flat" 		 	:b=new OM.Places.Shelter(ni,ni,50,25,15,31,true,true,true,"FlatYard"); 									inhPl( b,par);return b;
	case "FlatClub" 	 	:b=new OM.Places.Room(ni,ni,18,18,0,2.5,true,true,false,"Flat"); 										inhPl( b,par);return b;
	case "FlatApt1" 	 	:b=new OM.Places.Room(ni,ni,10,5,0,2.5,true,true,false,"Flat"); 											inhPl( b,par);return b;
 	case "FlatApt2" 	 	:b=new OM.Places.Room(ni,ni,10,5,0,2.5,true,true,false,"Flat"); 											inhPl( b,par);return b;
 	case "FlatApt3" 	 	:b=new OM.Places.Room(ni,ni,10,5,0,2.5,true,true,false,"Flat"); 											inhPl( b,par);return b;
 	case "Flat2StoryTerrace":b=new OM.Places.Shelter(ni,ni,50,2,3,6,true,true,true,"Flat"); 											inhPl( b,par);return b;
 	case "Flat2StoryStairs" :b=new OM.Places.Way(ni,ni,4,6,0,6,true,true,true,"Flat",["FlatYard","Flat2StoryTerrace"]);				inhPl( b,par);return b;
 	case "FlatApt4" 	 	:b=new OM.Places.Room(ni,ni,10,5,3,5.5,true,true,false,"Flat",["Flat2StoryTerrace"]); 					inhPl( b,par);return b;
 	case "FlatApt5" 	 	:b=new OM.Places.Room(ni,ni,10,5,3,5.5,true,true,false,"Flat",["Flat2StoryTerrace"]);					inhPl( b,par);return b;
 	case "FlatApt6" 	 	:b=new OM.Places.Room(ni,ni,10,5,3,5.5,true,true,false,"Flat",["Flat2StoryTerrace"]); 					inhPl( b,par);return b;
 	case "FlatApt7" 	 	:b=new OM.Places.Room(ni,ni,10,5,3,5.5,true,true,false,"Flat",["Flat2StoryTerrace"]);					inhPl( b,par);return b;
 	case "FlatClubDoor"  	:b=new OM.Places.Gate(ni,ni,0.4,0.8,0,2.5,true,true,false,"Flat",["FlatYard","FlatClub"],false); 		inhPl( b,par);return b;
 	case "FlatApt1Door"  	:b=new OM.Places.Gate(ni,ni,0.4,0.8,0,2.5,true,true,false,"Flat",["FlatYard","FlatApt1"],true);			inhPl( b,par);return b;
 	case "FlatApt2Door"  	:b=new OM.Places.Gate(ni,ni,0.4,0.8,0,2.5,true,true,false,"Flat",["FlatYard","FlatApt2"],true); 			inhPl( b,par);return b;
 	case "FlatApt3Door"  	:b=new OM.Places.Gate(ni,ni,0.4,0.8,0,2.5,true,true,false,"Flat",["FlatYard","FlatApt3"],true); 			inhPl( b,par);return b;
 	case "FlatApt4Door"  	:b=new OM.Places.Gate(ni,ni,0.4,0.8,0,2.5,true,true,false,"Flat",["Flat2StoryTerrace","FlatApt4"],true); inhPl( b,par);return b;
 	case "FlatApt5Door"  	:b=new OM.Places.Gate(ni,ni,0.4,0.8,0,2.5,true,true,false,"Flat",["Flat2StoryTerrace","FlatApt5"],true); inhPl( b,par);return b;
 	case "FlatApt6Door"  	:b=new OM.Places.Gate(ni,ni,0.4,0.8,0,2.5,true,true,false,"Flat",["Flat2StoryTerrace","FlatApt6"],true); inhPl( b,par);return b;
 	case "FlatApt7Door"  	:b=new OM.Places.Gate(ni,ni,0.4,0.8,0,2.5,true,true,false,"Flat",["Flat2StoryTerrace","FlatApt7"],true); inhPl( b,par);return b;

	case "FinVilla" 	 	:b=new OM.Places.Area(ni,ni,100,120,3,33,true,true,true,"Land"); 										inhPl( b,par);return b;
	case "FinRoad" 	 		:b=new OM.Places.Way(ni,ni,50,5,3,33,false,false,true,"Land",["Intersection","FinVilla"]); 				inhPl( b,par);return b;
	case "FinHouse" 	 	:b=new OM.Places.Shelter(ni,ni,30,20,15,33,true,true,true,"FinVilla");									inhPl( b,par);return b;
	case "FinHall" 	 		:b=new OM.Places.Room(ni,ni,3,5,3,6,true,true,false,"FinHouse"); 										inhPl( b,par);return b;
	case "FinDownstairs"	:b=new OM.Places.Way(ni,ni,6,1.5,0,6,true,true,false,"FinHouse",["FinCellar"]);						inhPl( b,par);return b;
	case "FinCellar" 	 	:b=new OM.Places.Room(ni,ni,10,10,0,2.5,true,true,false,"FinHouse"); 									inhPl( b,par);return b;
	case "FinBedroom" 	 	:b=new OM.Places.Room(ni,ni,6,5,3,6,true,true,false,"FinHouse"); 										inhPl( b,par);return b;
	case "FinKidsRoom"  	:b=new OM.Places.Room(ni,ni,6,5,3,6,true,true,false,"FinHouse"); 										inhPl( b,par);return b;
	case "FinLivingRoom"	:b=new OM.Places.Room(ni,ni,6,5,3,6,true,true,false,"FinHouse"); 										inhPl( b,par);return b;
	case "FinFrontDoor" 	:b=new OM.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"FinHouse",["FinVilla","FinHall"],true);	inhPl( b,par);return b;
	case "FinCellarDoor"	:b=new OM.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"FinHouse",["FinHall","FinDownstairs"]); 	inhPl( b,par);return b;
	case "FinBedroomDoor"	:b=new OM.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"FinHouse",["FinHall","FinBedroom"]);		inhPl( b,par);return b;
	case "FinKidsRoomDoor"	:b=new OM.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"FinHouse",["FinHall","FinKidsRoom"]);		inhPl( b,par);return b;
	case "FinLivingRoomWay" :b=new OM.Places.Way(ni,ni,0.4,0.8,3,5.5,false,true,false,"FinHouse",["FinHall","FinLivingRoom"]); 	inhPl( b,par);return b;

	case "SweVilla" 	 	:b=new OM.Places.Area(ni,ni,100,120,3,33,true,true,true,"Land"); 										inhPl( b,par);return b;
	case "SweRoad" 	 	 	:b=new OM.Places.Way(ni,ni,50,5,3,33,false,false,true,"Land",["Intersection","SweVilla"]); 				inhPl( b,par);return b;
	case "SweHouse" 		:b=new OM.Places.Shelter(ni,ni,30,20,15,33,true,true,true,"SweVilla");									inhPl( b,par);return b;
	case "SweHall" 	 		:b=new OM.Places.Room(ni,ni,3,5,3,6,true,true,false,"SweHouse"); 										inhPl( b,par);return b;
	case "SweDownstairs"	:b=new OM.Places.Way(ni,ni,6,1.5,0,6,true,true,false,"SweHouse",["SweCellar"]);							inhPl( b,par);return b;
	case "SweCellar" 		:b=new OM.Places.Room(ni,ni,10,10,0,2.5,true,true,false,"SweHouse"); 									inhPl( b,par);return b;
	case "SweBedroom" 		:b=new OM.Places.Room(ni,ni,6,5,3,6,true,true,false,"SweHouse"); 										inhPl( b,par);return b;
	case "SweKidsRoom"  	:b=new OM.Places.Room(ni,ni,6,5,3,6,true,true,false,"SweHouse"); 										inhPl( b,par);return b;
	case "SweLivingRoom"	:b=new OM.Places.Room(ni,ni,6,5,3,6,true,true,false,"SweHouse"); 										inhPl( b,par);return b;
	case "SweFrontDoor" 	:b=new OM.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"SweHouse",["SweVilla","SweHall"],true);		inhPl( b,par);return b;
	case "SweCellarDoor"	:b=new OM.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"SweHouse",["SweHall","SweDownstairs"]);		inhPl( b,par);return b;
	case "SweBedroomDoor"	:b=new OM.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"SweHouse",["SweHall","SweBedroom"]);			inhPl( b,par);return b;
	case "SweKidsRoomDoor"	:b=new OM.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"SweHouse",["SweHall","SweKidsRoom"]); 		inhPl( b,par);return b;
	case "SweLivingRoomWay"	:b=new OM.Places.Way(ni,ni,0.4,0.8,3,5.5,false,true,false,"SweHouse",["SweHall","SweLivingRoom"]); 		inhPl(b,par);return b}}
function inhPl(){}
OM.Places = { seed: ["Sky","Sea","Land", 	"Sandbox",
...
function precalcDefaultPatterns(seed,par){ var p=null; var ps=[];
	for(var n=0; n<OM.pattsL; n++){ p=mkPattern(seed[n],par); ps.push(p)}return ps}
