...
FL.Names.Fxsw1L=FL.Names.Fxsw1.length; FL.Names.fxsw1L=FL.Names.fxsw1.length; FL.Names.xxsx2L=FL.Names.xxsx2.length; FL.Names.FKSw1L=FL.Names.FKSw1.length; FL.Names.FkSw1L=FL.Names.FkSw1.length; FL.Names.FxxW1L=FL.Names.FxxW1.length; FL.Names.fKSw1L=FL.Names.fKSw1.length; FL.Names.fkSw1L=FL.Names.fkSw1.length; FL.Names.fxxW1L=FL.Names.fxxW1.length; FL.Names.xxSx2L=FL.Names.xxSx2.length;
...
function mkName2(o,fem,kid,fin,weird,last,s){ o.fem =fem; o.kid=kid; o.fin=fin; o.weird=weird;
	if 		(!fem && 		 fin &&  weird) { o.first =FL.Names.fxxW1[Math.floor(Math.random()*FL.Names.fxxW1L)]}
	else if (!fem && !kid && fin && !weird) { o.first =FL.Names.fkSw1[Math.floor(Math.random()*FL.Names.fkSw1L)]}
	else if (!fem &&  kid && fin && !weird) { o.first =FL.Names.fKSw1[Math.floor(Math.random()*FL.Names.fKSw1L)]}
 	else if (!fem &&  		!fin &&  weird) { o.first =FL.Names.fxxW1[Math.floor(Math.random()*FL.Names.fxxW1L)]}
 	else if (!fem &&   		!fin && !weird) { o.first =FL.Names.fxsw1[Math.floor(Math.random()*FL.Names.fxsw1L)]}
 	else if ( fem && 		 fin &&  weird) { o.first =FL.Names.FxxW1[Math.floor(Math.random()*FL.Names.FxxW1L)]}
	else if ( fem && !kid && fin && !weird) { o.first =FL.Names.FkSw1[Math.floor(Math.random()*FL.Names.FkSw1L)]}
	else if ( fem &&  kid && fin && !weird) { o.first =FL.Names.FKSw1[Math.floor(Math.random()*FL.Names.FKSw1L)]}
 	else if ( fem &&  		!fin &&  weird) { o.first =FL.Names.FxxW1[Math.floor(Math.random()*FL.Names.FxxW1L)]}
 	else if ( fem &&   		!fin && !weird) { o.first =FL.Names.Fxsw1[Math.floor(Math.random()*FL.Names.Fxsw1L)]}
 	if(last){ o.last =last}else{if(fin){ 	o.last  =FL.Names.xxSx2[Math.floor(Math.random()*FL.Names.xxSx2L)]}
 								else{ 		o.last 	=FL.Names.xxsx2}}
...
function mkDefaultPlaces(s,par){ var ps=[]; var p =null;
	for(var n=0; n<FL.PlacesL; n++){ p=mkPlace(FL.Places.seed[n],par); ps.push(p)} return ps}
function mkRandPlaces(s,par){ var ps=[]; var p =null;
	for(var n=0; n<FL.PlacesL; n++){ p=mkPlace(FL.Places.seed[n],par); ps.push(p)} return ps}
function mkPlace(ni,par){ var b =null; switch(ni){
	case "Sky" 			 	:b=new FL.Places.Sky(ni,12742000,12742000,0,10000,false,false,true,false,["Sea","Land"]); 				inhPl( b,par);return b;
	case "Sea" 		 		:b=new FL.Places.Sea(ni,12742000,12742000,-5000,0,false,false,true,false,["Sky","Land"]);				inhPl( b,par);return b;
	case "Land" 		 	:b=new FL.Places.Land(ni,7356597,7356597,-70000,5000,false,false,false,false,"Sea",["Sky","Sea"]);	inhPl( b,par);return b;
														//5385402
	case "Sandbox" 			:b=new FL.Places.Shelter(ni,ni,66,66,1,5,false,false,false,"Sandbox");									inhPl( b,par);return b; 			

	case "Beach" 		 	:b=new FL.Places.Area(ni,ni,300,75,-2,4,false,false,true,"Land");
	case "Market" 		 	:b=new FL.Places.Area(ni,ni,200,50,1,5,false,false,true,"Land",["Beach"]);								inhPl( b,par);return b;
	case "Inn" 				:b=new FL.Places.Shelter(ni,ni,25,25,1,5,false,false,false,"Land"); 										inhPl( b,par);return b;
	case "InnYard" 	 		:b=new FL.Places.Area(ni,ni,25,10,1,5,false,false,true,"Land",["Beach","Inn"]);							inhPl( b,par);return b;
	case "Esplanade" 		:b=new FL.Places.Way(ni,ni,25,5,1,5,false,false,true,"Land",["Market","InnYard","Beach"]); 				inhPl( b,par);return b;
	case "InnInterior" 		:b=new FL.Places.Room(ni,ni,24,9,0,4,true,false,false,"Inn"); 											inhPl( b,par);return b;
	case "InnDoor" 	 		:b=new FL.Places.Gate(ni,ni,0.4,0.8,0,2.5,false,false,false,"Inn",["InnYard","InnInterior"]);			inhPl( b,par);return b;

	case "ObeliskYard" 		:b=new FL.Places.Area(ni,ni,40,40,8,16,false,false,true,"Land"); 										inhPl( b,par);return b;
	case "Obelisk" 			:b=new FL.Places.Area(ni,ni,1,1,15140,15240,false,false,true,"ObeliskYard"); 							inhPl( b,par);return b;
	case "BoulevardBeach" 	:b=new FL.Places.Way(ni,ni,150,10,1,16,false,false,true,"Land",["Market","ObeliskYard"]); 				inhPl( b,par);return b;
	case "Garden" 		 	:b=new FL.Places.Area(ni,ni,200,200,16,25,false,false,true,"Land"); 										inhPl( b,par);return b;
	case "Temple" 	 	 	:b=new FL.Places.Shelter(ni,ni,88,66,21,25,false,false,true,"Garden"); 									inhPl( b,par);return b;
	case "TempleInterior"	:b=new FL.Places.Room(ni,ni,87,65,0,5,false,false,false,"Temple"); 										inhPl( b,par);return b;
	case "TempleDoor" 	 	:b=new FL.Places.Gate(ni,ni,0.5,1,0,3,false,false,false,"Temple",["Garden","TempleInterior"]); 			inhPl( b,par);return b;
	case "Intersection"  	:b=new FL.Places.Area(ni,ni,10,5,16,25,false,false,true,"Land",["Garden"]);								inhPl( b,par);return b;
	case "BoulevardInland" 	:b=new FL.Places.Way(ni,ni,90,10,16,25,false,false,true,"Land",["ObeliskYard","Intersection"]); 			inhPl( b,par);return b;

	case "FlatYard" 	 	:b=new FL.Places.Area(ni,ni,60,50,2,25,true,true,true,"Land",["ObeliskYard"]); 							inhPl( b,par);return b;
	case "Flat" 		 	:b=new FL.Places.Shelter(ni,ni,50,25,15,31,true,true,true,"FlatYard"); 									inhPl( b,par);return b;
	case "FlatClub" 	 	:b=new FL.Places.Room(ni,ni,18,18,0,2.5,true,true,false,"Flat"); 										inhPl( b,par);return b;
	case "FlatApt1" 	 	:b=new FL.Places.Room(ni,ni,10,5,0,2.5,true,true,false,"Flat"); 											inhPl( b,par);return b;
 	case "FlatApt2" 	 	:b=new FL.Places.Room(ni,ni,10,5,0,2.5,true,true,false,"Flat"); 											inhPl( b,par);return b;
 	case "FlatApt3" 	 	:b=new FL.Places.Room(ni,ni,10,5,0,2.5,true,true,false,"Flat"); 											inhPl( b,par);return b;
 	case "Flat2StoryTerrace":b=new FL.Places.Shelter(ni,ni,50,2,3,6,true,true,true,"Flat"); 											inhPl( b,par);return b;
 	case "Flat2StoryStairs" :b=new FL.Places.Way(ni,ni,4,6,0,6,true,true,true,"Flat",["FlatYard","Flat2StoryTerrace"]);				inhPl( b,par);return b;
 	case "FlatApt4" 	 	:b=new FL.Places.Room(ni,ni,10,5,3,5.5,true,true,false,"Flat",["Flat2StoryTerrace"]); 					inhPl( b,par);return b;
 	case "FlatApt5" 	 	:b=new FL.Places.Room(ni,ni,10,5,3,5.5,true,true,false,"Flat",["Flat2StoryTerrace"]);					inhPl( b,par);return b;
 	case "FlatApt6" 	 	:b=new FL.Places.Room(ni,ni,10,5,3,5.5,true,true,false,"Flat",["Flat2StoryTerrace"]); 					inhPl( b,par);return b;
 	case "FlatApt7" 	 	:b=new FL.Places.Room(ni,ni,10,5,3,5.5,true,true,false,"Flat",["Flat2StoryTerrace"]);					inhPl( b,par);return b;
 	case "FlatClubDoor"  	:b=new FL.Places.Gate(ni,ni,0.4,0.8,0,2.5,true,true,false,"Flat",["FlatYard","FlatClub"],false); 		inhPl( b,par);return b;
 	case "FlatApt1Door"  	:b=new FL.Places.Gate(ni,ni,0.4,0.8,0,2.5,true,true,false,"Flat",["FlatYard","FlatApt1"],true);			inhPl( b,par);return b;
 	case "FlatApt2Door"  	:b=new FL.Places.Gate(ni,ni,0.4,0.8,0,2.5,true,true,false,"Flat",["FlatYard","FlatApt2"],true); 			inhPl( b,par);return b;
 	case "FlatApt3Door"  	:b=new FL.Places.Gate(ni,ni,0.4,0.8,0,2.5,true,true,false,"Flat",["FlatYard","FlatApt3"],true); 			inhPl( b,par);return b;
 	case "FlatApt4Door"  	:b=new FL.Places.Gate(ni,ni,0.4,0.8,0,2.5,true,true,false,"Flat",["Flat2StoryTerrace","FlatApt4"],true); inhPl( b,par);return b;
 	case "FlatApt5Door"  	:b=new FL.Places.Gate(ni,ni,0.4,0.8,0,2.5,true,true,false,"Flat",["Flat2StoryTerrace","FlatApt5"],true); inhPl( b,par);return b;
 	case "FlatApt6Door"  	:b=new FL.Places.Gate(ni,ni,0.4,0.8,0,2.5,true,true,false,"Flat",["Flat2StoryTerrace","FlatApt6"],true); inhPl( b,par);return b;
 	case "FlatApt7Door"  	:b=new FL.Places.Gate(ni,ni,0.4,0.8,0,2.5,true,true,false,"Flat",["Flat2StoryTerrace","FlatApt7"],true); inhPl( b,par);return b;

	case "FinVilla" 	 	:b=new FL.Places.Area(ni,ni,100,120,3,33,true,true,true,"Land"); 										inhPl( b,par);return b;
	case "FinRoad" 	 		:b=new FL.Places.Way(ni,ni,50,5,3,33,false,false,true,"Land",["Intersection","FinVilla"]); 				inhPl( b,par);return b;
	case "FinHouse" 	 	:b=new FL.Places.Shelter(ni,ni,30,20,15,33,true,true,true,"FinVilla");									inhPl( b,par);return b;
	case "FinHall" 	 		:b=new FL.Places.Room(ni,ni,3,5,3,6,true,true,false,"FinHouse"); 										inhPl( b,par);return b;
	case "FinDownstairs"	:b=new FL.Places.Way(ni,ni,6,1.5,0,6,true,true,false,"FinHouse",["FinCellar"]);						inhPl( b,par);return b;
	case "FinCellar" 	 	:b=new FL.Places.Room(ni,ni,10,10,0,2.5,true,true,false,"FinHouse"); 									inhPl( b,par);return b;
	case "FinBedroom" 	 	:b=new FL.Places.Room(ni,ni,6,5,3,6,true,true,false,"FinHouse"); 										inhPl( b,par);return b;
	case "FinKidsRoom"  	:b=new FL.Places.Room(ni,ni,6,5,3,6,true,true,false,"FinHouse"); 										inhPl( b,par);return b;
	case "FinLivingRoom"	:b=new FL.Places.Room(ni,ni,6,5,3,6,true,true,false,"FinHouse"); 										inhPl( b,par);return b;
	case "FinFrontDoor" 	:b=new FL.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"FinHouse",["FinVilla","FinHall"],true);	inhPl( b,par);return b;
	case "FinCellarDoor"	:b=new FL.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"FinHouse",["FinHall","FinDownstairs"]); 	inhPl( b,par);return b;
	case "FinBedroomDoor"	:b=new FL.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"FinHouse",["FinHall","FinBedroom"]);		inhPl( b,par);return b;
	case "FinKidsRoomDoor"	:b=new FL.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"FinHouse",["FinHall","FinKidsRoom"]);		inhPl( b,par);return b;
	case "FinLivingRoomWay" :b=new FL.Places.Way(ni,ni,0.4,0.8,3,5.5,false,true,false,"FinHouse",["FinHall","FinLivingRoom"]); 	inhPl( b,par);return b;

	case "SweVilla" 	 	:b=new FL.Places.Area(ni,ni,100,120,3,33,true,true,true,"Land"); 										inhPl( b,par);return b;
	case "SweRoad" 	 	 	:b=new FL.Places.Way(ni,ni,50,5,3,33,false,false,true,"Land",["Intersection","SweVilla"]); 				inhPl( b,par);return b;
	case "SweHouse" 		:b=new FL.Places.Shelter(ni,ni,30,20,15,33,true,true,true,"SweVilla");									inhPl( b,par);return b;
	case "SweHall" 	 		:b=new FL.Places.Room(ni,ni,3,5,3,6,true,true,false,"SweHouse"); 										inhPl( b,par);return b;
	case "SweDownstairs"	:b=new FL.Places.Way(ni,ni,6,1.5,0,6,true,true,false,"SweHouse",["SweCellar"]);							inhPl( b,par);return b;
	case "SweCellar" 		:b=new FL.Places.Room(ni,ni,10,10,0,2.5,true,true,false,"SweHouse"); 									inhPl( b,par);return b;
	case "SweBedroom" 		:b=new FL.Places.Room(ni,ni,6,5,3,6,true,true,false,"SweHouse"); 										inhPl( b,par);return b;
	case "SweKidsRoom"  	:b=new FL.Places.Room(ni,ni,6,5,3,6,true,true,false,"SweHouse"); 										inhPl( b,par);return b;
	case "SweLivingRoom"	:b=new FL.Places.Room(ni,ni,6,5,3,6,true,true,false,"SweHouse"); 										inhPl( b,par);return b;
	case "SweFrontDoor" 	:b=new FL.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"SweHouse",["SweVilla","SweHall"],true);		inhPl( b,par);return b;
	case "SweCellarDoor"	:b=new FL.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"SweHouse",["SweHall","SweDownstairs"]);		inhPl( b,par);return b;
	case "SweBedroomDoor"	:b=new FL.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"SweHouse",["SweHall","SweBedroom"]);			inhPl( b,par);return b;
	case "SweKidsRoomDoor"	:b=new FL.Places.Gate(ni,ni,0.4,0.8,3,5.5,false,true,false,"SweHouse",["SweHall","SweKidsRoom"]); 		inhPl( b,par);return b;
	case "SweLivingRoomWay"	:b=new FL.Places.Way(ni,ni,0.4,0.8,3,5.5,false,true,false,"SweHouse",["SweHall","SweLivingRoom"]); 		inhPl(b,par);return b}}
function inhPl(){}
FL.Places = { seed: ["Sky","Sea","Land", 	"Sandbox",
...
function mkDefaultSubjects(sim){	var r =[]; var b =null;	
	b =mkReSubj( "subj_A",sim,AB.sQr, 1,1,1);r.push( b);
	b =mkReSubj( "subj_a",sim,AB.sQr, 1,1,0);r.push( b);
	b =mkReSubj( "subj_B",sim,AB.sQr, 0,1,1);r.push( b);
	b =mkReSubj( "subj_b",sim,AB.sQr, 0,1,0);r.push( b);
	b =mkReSubj( "subj_C",sim,AB.sQr, 1,0,1);r.push( b);
	b =mkReSubj( "subj_c",sim,AB.sQr, 1,0,0);r.push( b);
	b =mkReSubj( "subj_D",sim,AB.sQr, 0,0,1);r.push( b);
	b =mkReSubj( "subj_d",sim,AB.sQr, 0,0,0);r.push( b);
	b =mkReSubj( "subj_E",sim,AB.sQr, 1,1,1);r.push( b);
	b =mkReSubj( "subj_e",sim,AB.sQr, 1,1,0);r.push( b);
	b =mkReSubj( "subj_F",sim,AB.sQr, 0,1,1);r.push( b);
	b =mkReSubj( "subj_f",sim,AB.sQr, 0,1,0);r.push( b);
	b =mkReSubj( "subj_G",sim,AB.sQr, 1,0,1);r.push( b);
	b =mkReSubj( "subj_g",sim,AB.sQr, 1,0,0);r.push( b);
	b =mkReSubj( "subj_H",sim,AB.sQr, 0,0,1);r.push( b);
	b =mkReSubj( "subj_h",sim,AB.sQr, 0,0,0);r.push( b);
	return r}
function mkRandSubjects(sim){	var r =mkDefaultSubjects(sim); return r}
/*
	s.pMd =AB.mDs.filter( mD => mD.sex.fem ===s.fem)
...
function mkReSubj( ni,sim,reso,mas,Ac,ppl,tY,fN,cR,pM,stereo,bAge,gAge,queer,queerBG,loc,dir,obj){ var b;
	if( !ni){ 	var ni=""}
	if( !reso){ var reso=FL.pcReso}
	if( !mas){ 	var mas =FL.pcMas} 	b =Math.random(); b <mas ? mas =true  : mas =false;
	if( !Ac){ 	var Ac  =FL.pcAc} 	b =Math.random(); b <Ac  ? Ac  =true  : Ac  =false;
	if( !ppl){ 	var ppl =FL.pcPpl} 	b =Math.random(); b <ppl ? ppl =true  : ppl =false; 
	if( !(stereo >0)){ var stereo =false; b =Math.random();
		if(!Ac){if( !bAge){var bAge =FL.pcBAge}
			if(b <bAge){  		bAge =true;  gAge =false; var bQqueer =false; var gQueer =false}
			else{ if(!queer){ 	var queer =bAge +FL.pcQueer} else{ var queer =bAge +queer}
			 	if(b <queer){ 	b =Math.random();
					if(!queerBG){ 	var queerBG =FL.pcQueerBG}
		...
		else{ 	if( !gAge){var gAge =FL.pcGAge}
			if(b <gAge){  		bAge =false; gAge =true;  var bQqueer =false; var gQueer =false}
			else{ if(!queer){ 	var queer =gAge +FL.pcQueer} else{ var queer =gAge +queer}
			 	if(b <queer){ 	b =Math.random();
					if(!queerBG){ 	var queerBG =FL.pcQueerBG}
					if(b <queerBG){ bAge =false; gAge =false; var bQueer =true;  var gQueer =false}
					else{ 			bAge =false; gAge =false; var bQueer =false; var gQueer =true}}
	...
	if( !tY){ var tY =FL.pcTys}	if( tY.length===0){ tY =FL.Sociotypes.seed}				else{ tY =union( tY, FL.Sociotypes.seed)}
	tY =tY[Math.floor(Math.random() *tY.length)];
	if( !fN){ var fN =FL.pcFns}	if( fN.length===0){ fN =FL.Functions.seed} 				else{ fN =union( fN, FL.Functions.seed)}
	fN =fN[Math.floor(Math.random() *fN.length)];
	if( !cR){ var cR =FL.pcCrs}	if( cR.length===0){ cR =FL.Circuits.seed}				else{ cR =union( cR, FL.Circuits.seed)}
	cR =cR[Math.floor(Math.random() *cR.length)];

	if( !pM){ var pM =FL.pcPms}	if( pM.length===0){ pM =FL.PrimalMoods.seed}			else{ pM =union( pM, FL.PrimalMoods.seed)}
	pM =pM[Math.floor(Math.random() *pM.length)];

	//if( !xtn){ var xtn =FL.pcXtns}	if( xtn.length===0){ xtn =FL.VectorQualities.seed}	else{ xtn =union( xtn, FL.VectorQualities.seed)}
	///*filter vQs by function IE*/xtn =xtn[Math.floor(Math.random() *xtn.length)];
	//if( !ntn){ var ntn =FL.pcNtns}	if( ntn.length===0){ ntn =FL.VectorQualities.seed}	else{ ntn =union( ntn, FL.VectorQualities.seed)}
	///*filter vQs by function IE*/ntn =ntn[Math.floor(Math.random() *ntn.length)];

	if (!dir){ var dir = { x:0, y:0}}
	if (!loc){ var loc = { x:0, y:0}}
	/*if( !){ 			var nD =FL.pcNd} b <nD ? nD =true : nD =false;
	if( !nD){ if( !cD){ var cD =FL.pcCd} b <cD ? cD =true : cD =false;}
	var dir={ x:0, y:0};
	if( nD){ if( !nDs){ var nDs =FL.pcNds} b <nDR ? nDR =true : nDR =false;
		if( nDR){ 	nDR=[]; for(var i=FL.pcNdx; !i>FL.pcNdX; i++){ nDR.push(i)} dir.x =nDR[Math.floor(Math.random() *nDR.length)];
					nDR=[]; for(var i=FL.pcNdy; !i>FL.pcNdY; i++){ nDR.push(i)} dir.y =nDR[Math.floor(Math.random() *nDR.length)]}}
	else if( cD){ }

		if( dir.length===0){ dir =FL.MosaicDirections.seed}	else{ dir =union( dir, FL.MosaicDirections.seed)}
	dir =dir[Math.floor(Math.random() *dir.length)];*/

	if( !obj){ var obj	=FL.pcObj} 		Math.random() <obj ? obj =true : obj =false;

	var o =new Subj( ni,sim,reso,mas,Ac,ppl,tY,fN,cR,pM,stereo,bAge,gAge,bQueer,gQueer,loc,dir,obj);
	sim.subjsL++;
	/*if( FL.sBTunes[2]==FL.sim.subjsL ){ FL.sBTunes[3] =FL.sim.subjs.length+1}*/
   	return o}
...
function precalcDefaultPatterns(seed,par){ var p=null; var ps=[];
	for(var n=0; n<FL.pattsL; n++){ p=mkPattern(seed[n],par); ps.push(p)}return ps}
